...
import (
	...
	"github.com/Nahom7wos/Airlines-Booking-System/form"
...
func (auh *AdminUserHandler) AdminUser(w http.ResponseWriter, r *http.Request) {
	...
	auh.tmpl.ExecuteTemplate(w, "admin.user.layout", tmplData)
}
...
func (auh *AdminUserHandler) AdminStoreUser(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		accountForm.Required("fullName", "email","passport", "username", "password", "confirmPassword")
		...
		if pExists {
			upAccForm.VErrors.Add("passport", "Passport Already Exists")
...
func (auh *AdminUserHandler) AdminUpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		usr, errs := auh.userService.User(uint(id))
		if len(errs) > 0 {
			http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
			return
		}
		lgn, errs := auh.loginService.LoginByUserId(usr.ID)
		if len(errs) > 0 {
			http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
			return
		}
		...
		values.Add("fullName", usr.FullName)
		values.Add("email", usr.Email)
		Values.Add("passport", usr.Passport)
	...
	if r.Method == http.MethodPost {
		...
		upAccForm.Required("fullName", "email","passport", "username", "password", "confirmPassword")
		...
		pExists := auh.userService.PassportExists(r.FormValue("passport"))
		if (getUsr.Passport != r.FormValue("passport")) && pExists {
			upAccForm.VErrors.Add("passport", "Passport Already Exists")
			auh.tmpl.ExecuteTemplate(w, "admin.user.update.layout", upAccForm)
			return
		}
